...
class NewBookingForm extends React.Component {
  ...
  addGuest(e) {
    e.preventDefault();
    if (this.state.num_guests < 6) this.setState({ num_guests: this.state.num_guests + 1 });
  }

  substractGuest(e) {
    e.preventDefault();
    if (this.state.num_guests > 1) this.setState({ num_guests: this.state.num_guests - 1 });
  }
  ...
  renderTotalGuest() {
    const guestCount = this.state.num_guests > 1 ? (this.state.num_guests - 1 )/ 2.0 * this.props.spot.price: 0;
    const guestTotal = (this.handleDate(this.state.endDate).split('/')[1] - this.handleDate(this.state.startDate).split('/')[1]) * this.props.spot.price;
    return guestCount + guestTotal
  }
      ...
      <div className={scroll} id="shadow">   
        <div className="booking-header">
          ...
          <div className="rating-container-booking">
              <div className="star"><i className="fas fa-star"></i></div>{this.props.rating}</div>
          </div>
        ...
        <div className="booking-gem">
          <div className="booking-extra-container">
            ...
            <div className="gem"><i className="far fa-gem"></i></div>
          </div>
        </div>
        <div className="total-price-booking">
          <span>Total</span>
          <div className="booking-total">${this.renderTotalGuest() ? this.renderTotalGuest() : 0}</div>
        </div>
